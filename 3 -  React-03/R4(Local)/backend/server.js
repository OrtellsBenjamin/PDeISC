...
app.get("/api/hero", async (req, res) => {
  try {
    ...
    res.json(
      rows[0] || { heroTitle: "", heroText: "" } // retornar ambos campos
    );
  } catch (err) {
    console.error(err);
...
app.post("/api/hero", async (req, res) => {
  try {
    ...
    await db.query(
      "INSERT INTO hero (id, heroTitle, heroText) VALUES (1, ?, ?) ON DUPLICATE KEY UPDATE heroTitle=?, heroText=?",
      [heroTitle, heroText, heroTitle, heroText]
    );
