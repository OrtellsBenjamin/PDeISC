...
export default function Experience({ experienceRef, editingSection, setEditingSection, isLogged }) {
  ...
  // Cargar experiencias desde backend local
  useEffect(() => {
  ...
  const handleDeleteExperience = async (index) => {
    ...
    try {
      if (expToDelete.id) {
        await fetch(`http://localhost:3000/api/experience/${expToDelete.id}`, { method: "DELETE" });
      }
      const newList = [...experienceList];
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="relative border-l-2 border-gray-300 pl-8 md:pl-12">
          {experienceList.map((exp, i) => (
            <motion.div key={exp.id || i} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }} className="mb-10 relative">
              ...
              <div className="flex flex-col md:flex-row md:gap-6">
                ...
                <div className="md:flex-1 mt-2 md:mt-0">
                  ...
                  {exp.link && !editingSection && (
                    <a href={exp.link} className="text-blue-600 font-medium mt-1 inline-block">Saber más &gt;</a>
                  )}
                  {editingSection && (
                    <button onClick={() => setConfirmDelete(i)} className="mt-2 px-4 py-1 bg-red-500 text-white rounded hover:bg-red-600">
                      Eliminar
                    </button>
                  )}
                </div>
